...
ARG VERSION
ARG GIT_COMMIT
WORKDIR /go/src/github.com/platanus/bitcoin-prometheus-exporter
COPY *.go ./
COPY vendor ./vendor
...
FROM alpine:latest
COPY --from=builder /go/src/github.com/platanus/bitcoin-prometheus-exporter/exporter /usr/bin/
